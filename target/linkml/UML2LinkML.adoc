...
@startuml
class C
C : a<sub>1</sub>: t<sub>1</sub> [i<sub>1</sub>..j<sub>1</sub>]
C : a<sub>2</sub>: t<sub>2</sub> [i<sub>2</sub>..j<sub>2</sub>]
@enduml
....
...
| `cims:stereotype`
| `cims:stereotype`
| annotation: `cims:stereotype` footnote:[Some stereotypes -- such as `uml:attribute` are self-evident and can be trivially mapped by generators.]
...
* #TODO#: Document default cardinalities (when nothing is explitictly stated).
* `owned_by` for enum permissible values and class attributes is inherited from the enum and class (resp.) and therefore not explicitly stated.
** In fact, the metamodel does not seem to even support the `owned_by` slot for pv's.
